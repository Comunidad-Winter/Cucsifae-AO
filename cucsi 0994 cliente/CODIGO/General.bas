...
'[Alejo-21-5]
Public Sub Limite(ByRef Vari As Variant, ByVal Mini As Long, ByVal Maxi As Long)
If Vari < Mini Then Vari = Mini
If Vari > Maxi Then Vari = Maxi
End Sub

'[Alejo-21-5]
Sub BorraGenteTrasMio(ByVal nHead As Integer)
Dim X As Long, Y As Long
Dim xI As Long, xF As Long
Dim yI As Long, yF As Long

Select Case nHead
Case NORTH, SOUTH
    '***** GENTE VIEJA *****
    If nHead <> NORTH Then 'SOUTH
        yI = UserPos.Y - MinYBorder
        yF = yI
    Else 'NORTH
        yI = UserPos.Y + MinYBorder
        yF = yI
    ...
    'Call Limite(yF, 1, 100)
    
    xI = UserPos.X - MinXBorder ': Call Limite(xI, 1, 100)
    xF = UserPos.X + MinXBorder ': Call Limite(xF, 1, 100)
Case EAST, WEST
    '***** GENTE VIEJA *****
    If nHead <> EAST Then 'WEST
        xI = UserPos.X + MinXBorder
        xF = xI
    Else 'EAST
        xI = UserPos.X - MinXBorder
        xF = xI
    ...
    'Call Limite(xF, 1, 100)
    
    yI = UserPos.Y - MinYBorder ': Call Limite(yI, 1, 100)
    yF = UserPos.Y + MinYBorder ': Call Limite(yF, 1, 100)
Case Else
    Exit Sub
End Select
...
For Y = yI To yF
    For X = xI To xF
        If MapData(X, Y).CharIndex > 0 Then
            Call Dialogos.QuitarDialogo(MapData(X, Y).CharIndex)
            'Call AddtoRichTextBox(frmMain.RecTxt, "BGTM " & MapData(X, Y).CharIndex, 255, 255, 255, , , False)
            CharList(MapData(X, Y).CharIndex).POS.X = 0
            CharList(MapData(X, Y).CharIndex).POS.Y = 0
...
Select Case Dir
Case NORTH
    nPos.X = UserPos.X: nPos.Y = UserPos.Y - 1
Case EAST
    nPos.X = UserPos.X + 1: nPos.Y = UserPos.Y
Case SOUTH
    nPos.X = UserPos.X: nPos.Y = UserPos.Y + 1
Case WEST
    nPos.X = UserPos.X - 1: nPos.Y = UserPos.Y
...
If LegalPos(nPos.X, nPos.Y) Then
    'Call BorraGenteTrasMio(Dir)
    
    'Call SendData("M" & Dir)
    Call OkEnviarCaminar(Dir)
    'If Not UserDescansar And Not UserMeditar And Not UserParalizado Then
    If Not UserParalizado Then
        Call MoveCharbyHead(UserCharIndex, Dir)
        Call MoveScreen(Dir)
        Call DoFogataFx
    End If
Else
    If CharList(UserCharIndex).Heading <> Dir Then
            Call SendData("CHEA" & Dir)
    End If
End If
...
If UserMoving = 0 And OkPuedeCaminar() Then
...
'    ServersLst(Cont + 1).Ip = frmMain.Winsock1.LocalIP
'    ServersLst(Cont + 1).Puerto = "7666"
'    ServersLst(Cont + 1).desc = "Local"
'    ServersLst(Cont + 1).PassRecPort = "7667"
...
frmMain.Inet1.URL = "http://www.totalao.com/iplist.txt"
...
Do While prgRun
    ...
    If EngineRun Then
        ...
        '[END]'
            ...
            '[CODE 000]:MatuX
                ...
                If ISseguro Then Call Dialogos.DrawText(IIf(IScombate, 350, 260), 260, IIf(IScombate, " - SEGURO", "SEGURO"), vbRed)
    ...
    '[Alejo-21-5]: Por si sube el FPS, caminar siempre a 5 PPS
    If (GetTickCount - LastTime >= ConfEnv.IntOk) Or True Then
    ...
    ' Frame Limiter
        ...
        If DirectX.TickCount - lFrameTimer > 1000 Then
            ...
            If FPSFLAG Then frmMain.Caption = "FPS: " & FramesPerSec '& " Ping: " & Ping
...
Public Function EsUnaIp(dato As String) As Boolean
    Dim i As Integer
    For i = 1 To 4
        If IsNumeric(ReadField(i, dato, Asc("."))) Then
            If CInt(ReadField(i, dato, Asc("."))) > 255 Then
                EsUnaIp = False
                Exit Function
            End If
        Else
...
Public Function OkPuedeCaminar() As Boolean
If ConfEnv.ModoPaquetes = 1 Then
    If FlagOk.Cant < ConfEnv.IntOkCantPak Then
        OkPuedeCaminar = True
    Else
        OkPuedeCaminar = False
    End If
Else
    OkPuedeCaminar = True
End If
...
Public Sub OkEnviarCaminar(ByVal nHeading As Integer)
FlagOk.Cant = FlagOk.Cant + 1
Call SendData("M" & nHeading & FlagOk.NumMagico)

If FlagOk.Cant = ConfEnv.IntOkCantPak Then PingInicio = GetTickCount()
...
If Left$(Datos, 1) = "/" Then
    If UCase(Left$(Datos, 8)) = "/PASSWD " Then
            Dim j$
            j$ = MD5String(Right$(Datos, Len(Datos) - 8))
            Datos = "/PASSWD " & j$
    End If
    If UCase(Left(Datos, 5)) = "/PING" Then
        PingInicio = GetTickCount()
    End If
    Call SendData(Datos)

'Shout
ElseIf Left$(Datos, 1) = "-" Then
    Call SendData("-" & Right$(Datos, Len(Datos) - 1))
