...
Private Sub Form_Click()
    ...
    'Not CBool((GetKeyState(vbKeyShift) Or 1) Mod -127)
    'devuelve true si esta presionada la tecla
...
If (Not SendTxt.Visible) And _
   ...
   (KeyCode >= 48 And KeyCode <= 57)) Then
        ...
        Select Case KeyCode
            ...
            Case vbKeyF12:
                If tmrMacro.Enabled = False Then
                    tmrMacro.Enabled = True
                    Call AddtoRichTextBox(frmMain.RecTxt, "Macro Activado.", 255, 255, 255, 0, 0)
                    Else
                    tmrMacro.Enabled = False
                    Call AddtoRichTextBox(frmMain.RecTxt, "Macro Desactivado.", 255, 255, 255, 0, 0)
                End If
        End Select
End Sub
...
Private Sub Form_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If tX < XMinMapSize Or tX > XMaxMapSize Or tY < YMinMapSize Or tY > YMaxMapSize Then Exit Sub
    
    If uPos.X = tX And uPos.Y = tY Then Exit Sub
    uPos.X = tX: uPos.Y = tY
    
    'Debug.Print MapData(tX, tY).CharIndex & " " & tX & " " & tY
    If MapData(tX, tY).CharIndex > 0 Then
        If CharList(MapData(tX, tY).CharIndex).nombre = "" And Not CharList(MapData(tX, tY).CharIndex).soyNpc Then
            Debug.Print "Pedí"
            Call SendData("N" & tX & "," & tY)
        End If
        
        MostrarIndexNombre = MapData(tX, tY).CharIndex
    Else
        MostrarIndexNombre = 0
    End If
End Sub
...
Private Sub Socket1_Read(DataLength As Integer, IsUrgent As Integer)
    ...
    Dim binLongitud As Integer
    Dim binComando As String
    Dim binEOC As Boolean
    
    'Agrega inicio de cadena truncada (si es que lo hay)
    If TempString <> "" Then
        RD = TempString & RD
        TempString = ""
    End If

    'Interpreta la cadena recibida y la separa segun la cantidad
    'de caracteres por comando
    sChar = 1
    
    Do While Not binEOC
        
        binLongitud = CInt(binAsc2Dec(Mid$(RD, sChar, 2)))
        CR = CR + 1
        If (sChar + 2 + binLongitud) - 1 > Len(RD) Then
            TempString = Mid$(RD, sChar) 'Cadena truncada
            sChar = Len(RD) + 1
        Else
            rBuffer(CR) = Mid$(RD, sChar + 2, binLongitud)
            sChar = (sChar + 2 + binLongitud)
        End If
        If sChar - 1 = Len(RD) Then binEOC = True
...
Private Sub Winsock1_Connect()
    ...
    '[Alejo-21-5]
    ConfEnv.IntOk = 0
    ConfEnv.IntOkCantPak = 0
    ConfEnv.ModoPaquetes = 0
    FlagOk.Cant = 0
    FlagOk.NumMagico = 0
...
#Else
    ...
    Case Normal, CrearNuevoPj, Dados, BorrarPj
