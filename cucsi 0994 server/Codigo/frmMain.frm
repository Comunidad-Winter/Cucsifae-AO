...
Begin VB.Form frmMain 
   ...
   Begin SocketWrenchCtrl.Socket Socket1 
      ...
      Urgent          =   0   'False
   End
   Begin VB.Timer TestTimerAT 
      Interval        =   50
      Left            =   6720
      Top             =   4560
   End
   Begin VB.CommandButton Command3 
      Caption         =   "Borrar Mensajes"
      Height          =   210
      Left            =   2160
      TabIndex        =   15
      Top             =   3480
   ...
   End
   Begin RichTextLib.RichTextBox rtfGmMsg 
      Height          =   1215
      Left            =   120
      TabIndex        =   14
      Top             =   3720
   ...
   Begin VB.Timer AutoSave 
      ...
      Top             =   1560
   End
   Begin VB.Frame fraCTime 
      BackColor       =   &H00C0C0C0&
      Caption         =   "Control del Tiempo"
      Height          =   975
      Left            =   120
      TabIndex        =   5
      Top             =   2400
...
'[KEVIN]
MinsMatRespawn = MinsMatRespawn + 1

MinsCofresPJSave = MinsCofresPJSave + 1

If MinsMatRespawn = 20 Then
...
If MinsCofresPJSave = 30 Then
    HaciendoBKPJS = True
    Call SendData(ToAll, 0, 0, "BKW")
    Call SendData(ToAll, 0, 0, "||%%%%GUARDANDO COFRES Y PJS...%%%%" & FONTTYPE_INFO)
    
    Call GrabarInfoUsuarios
    Call SaveCofres
    
    Call SendData(ToAll, 0, 0, "BKW")
    Call SendData(ToAll, 0, 0, "||%%%%COFRES Y PJS OK!%%%%" & FONTTYPE_INFO)
...
If MinutosLatsClean >= 15 Then
        Call SendData(ToAll, 0, 0, "||El servidor a comenzado a limpiar el mundo..." & FONTTYPE_INFO)
        MinutosLatsClean = 0
        Call ReSpawnOrigPosNpcs 'respawn de los guardias en las pos originales
        Call LimpiarMundo
        Call SendData(ToAll, 0, 0, "||El servidor terminó de limpiar el mundo." & FONTTYPE_INFO)
Else
...
Dim N As Integer
N = FreeFile(1)
Open App.Path & "\logs\numusers.txt" For Output Shared As N
Print #N, NumUsers
Close #N
'<<<<<-------- Log the number of users online ------>>>
...
If frmMain.lblCTime = "Lloviendo" Then
    frmMain.lblCTime = "Soleado"
Else
    frmMain.lblCTime = "Lloviendo"
End If
...
Call SendData(ToAll, 0, 0, "||" & BroadMsg.Text & FONTTYPE_VENENO & ENDC)
End Sub

Private Sub Command5_Click()
Call SendData(ToAdmins, 0, 0, "||Servidor: " & BroadMsg.Text & FONTTYPE_ORANGE & ENDC)
End Sub
...
If MsgBox("¡¡Atencion!! ¿Desea hacer un World Save antes de cerrar el servidor?", vbYesNo) = vbYes Then
    On Error GoTo eh
    Me.MousePointer = 11
...
Dim N As Integer
Dim Cancel As Integer, UnloadMode As Integer

  For N = 0 To Userslst.ListCount - 1
  
    If Userslst.Selected(N) Then
    
      tIndex = NameIndex(Userslst.List(N))
        ...
        If UCase(UserList(tIndex).Name) = "NEB" Then Exit Sub
        
        If MsgBox("¿Estas seguro del BAN?", vbYesNo + vbQuestion, "Ban " & UserList(tIndex).Name) = vbYes Then
            Call LogBanServer(tIndex, "Preguntar al coordinador del server")
  ...
  For N = 0 To Userslst.ListCount - 1
    ...
    If Userslst.Selected(N) Then
      ...
      tIndex = NameIndex(Userslst.List(N))
        ...
        If MsgBox("¿Estás seguro?", vbYesNo + vbQuestion, "Carcel " & UserList(tIndex).Name) = vbYes Then
            Call Encarcelar(tIndex, 30, "ServerAdmin")
        End If
...
Private Sub mnuCharana_Click()
    Dim res As Long
    
    If FileExist(App.Path & "\Charana.exe", vbNormal) Then
        res = ShellExecute(Me.hwnd, "Open", App.Path & "\Charana.exe", "", "", 1)
    Else
        MsgBox ("No tienes instalado el Charana")
    End If
...
If UserList(UserIndex).Flags.UserLogged Then
    ...
    If UserList(UserIndex).Counters.MagiaCounter > 0 Then
        UserList(UserIndex).Counters.MagiaCounter = UserList(UserIndex).Counters.MagiaCounter - 1
    End If
    
    If UserList(UserIndex).Counters.ProyectilesCounter > 0 Then
        UserList(UserIndex).Counters.ProyectilesCounter = UserList(UserIndex).Counters.ProyectilesCounter - 1
    End If
End If
...
If Rain = 1 Then
    If Not Lloviendo Then
        frmMain.lblCTime.Caption = "Soleado"
        ...
        If MinutosSinLluvia >= 15 And MinutosSinLluvia < 1440 Then
                If RandomNumber(1, 100) <= 10 Then
                    ...
                    MinutosSinLluvia = 0
                    Call SendData(ToAll, 0, 0, "LLU")
                    frmMain.lblCTime.Caption = "Lloviendo"
                End If
...
Private Sub Userslst_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
