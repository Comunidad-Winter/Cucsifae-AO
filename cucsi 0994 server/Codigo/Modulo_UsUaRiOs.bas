...
If InMapBounds(Map, X, Y) Then
       ...
       'DIOSES------
       Dim God As Byte
       If EsDios(UserList(UserIndex).Name) Or EsSemiDios(UserList(UserIndex).Name) Then
        God = 1
       Else
        God = 0
       End If
       '------------
       If klan$ <> "" Then
            Call SendData(sndRoute, sndIndex, sndMap, "CC" & UserList(UserIndex).Char.Body & "," & UserList(UserIndex).Char.Head & "," & UserList(UserIndex).Char.Heading & "," & UserList(UserIndex).Char.CharIndex & "," & X & "," & Y & "," & UserList(UserIndex).Char.WeaponAnim & "," & UserList(UserIndex).Char.ShieldAnim & "," & UserList(UserIndex).Char.FX & "," & 999 & "," & UserList(UserIndex).Char.CascoAnim & "," & UserList(UserIndex).Name & " <" & klan$ & ">" & "," & bCr & "," & God)
       Else
            Call SendData(sndRoute, sndIndex, sndMap, "CC" & UserList(UserIndex).Char.Body & "," & UserList(UserIndex).Char.Head & "," & UserList(UserIndex).Char.Heading & "," & UserList(UserIndex).Char.CharIndex & "," & X & "," & Y & "," & UserList(UserIndex).Char.WeaponAnim & "," & UserList(UserIndex).Char.ShieldAnim & "," & UserList(UserIndex).Char.FX & "," & 999 & "," & UserList(UserIndex).Char.CascoAnim & "," & UserList(UserIndex).Name & "," & bCr & "," & God)
       End If
...
If UserList(UserIndex).Stats.Exp >= UserList(UserIndex).Stats.ELU Then
    ...
    If UserList(UserIndex).Invent.AnilloEqpSlot > 0 Then
        If ObjData(UserList(UserIndex).Invent.AnilloEqpObjIndex).TipoAnillo = 5 Then
            Call Desequipar(UserIndex, UserList(UserIndex).Invent.AnilloEqpSlot)
        End If
    End If
    
    If UserList(UserIndex).Invent.Anillo2EqpSlot > 0 Then
        If ObjData(UserList(UserIndex).Invent.Anillo2EqpObjIndex).TipoAnillo = 5 Then
...
If UserList(UserIndex).Invent.ArmourEqpObjIndex > 0 Then
    ...
    If ObjData(UserList(UserIndex).Invent.ArmourEqpObjIndex).DefMagia = 1 Then
        Call SendData(ToIndex, sendIndex, 0, "||(CUERPO) Magia Min Def/Max Def: " & ObjData(UserList(UserIndex).Invent.ArmourEqpObjIndex).MagiaMinDef & "/" & ObjData(UserList(UserIndex).Invent.ArmourEqpObjIndex).MagiaMaxDef & FONTTYPE_INFO)
    Else
        Call SendData(ToIndex, sendIndex, 0, "||(CUERPO) Magia Min Def/Max Def: 0" & FONTTYPE_INFO)
    End If
Else
...
If UserList(UserIndex).Invent.AnilloEqpObjIndex > 0 Then
    If ObjData(UserList(UserIndex).Invent.AnilloEqpObjIndex).TipoAnillo = 4 Then
        Call SendData(ToIndex, sendIndex, 0, "||(ANILLO) Def: " & ObjData(UserList(UserIndex).Invent.AnilloEqpObjIndex).MaxModificador & FONTTYPE_INFO)
    Else
        If ObjData(UserList(UserIndex).Invent.Anillo2EqpObjIndex).TipoAnillo = 4 Then
            Call SendData(ToIndex, sendIndex, 0, "||(ANILLO) Def: " & ObjData(UserList(UserIndex).Invent.Anillo2EqpObjIndex).MaxModificador & FONTTYPE_INFO)
        Else
            Call SendData(ToIndex, sendIndex, 0, "||(ANILLO) Def: 0" & FONTTYPE_INFO)
        End If
    End If
...
For Y = YMinMapSize To YMaxMapSize
    For X = XMinMapSize To XMaxMapSize
        ...
        If MapData(Map, X, Y).UserIndex > 0 And UserIndex <> MapData(Map, X, Y).UserIndex Then
            ...
            If UserList(MapData(Map, X, Y).UserIndex).Flags.Invisible = 1 Then Call SendData(ToIndex, UserIndex, 0, "KLBPA" & UserList(MapData(Map, X, Y).UserIndex).Char.CharIndex & ",1")
        End If
...
End Sub
'[KEVIN]
Sub DesequiparTodo(ByVal UserIndex As Integer)
