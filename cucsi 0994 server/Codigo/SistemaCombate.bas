...
If SkillTacticas = 0 Then SkillTacticas = 1
...
Public Function CalcularDaño(ByVal UserIndex As Integer, ByVal VictimIndex As Integer, Optional ByVal NpcIndex As Integer = 0) As Long
...
'[KEVIN]
MaxH = UserList(UserIndex).Stats.MaxHIT
MinH = UserList(UserIndex).Stats.MinHIT

If NpcIndex > 0 Then

    If Npclist(NpcIndex).Char.Heading = NORTH And UserList(UserIndex).Char.Heading = NORTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = SOUTH And UserList(UserIndex).Char.Heading = SOUTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = EAST And UserList(UserIndex).Char.Heading = EAST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = WEST And UserList(UserIndex).Char.Heading = WEST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = NORTH And UserList(UserIndex).Char.Heading = WEST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = NORTH And UserList(UserIndex).Char.Heading = EAST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = SOUTH And UserList(UserIndex).Char.Heading = WEST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = SOUTH And UserList(UserIndex).Char.Heading = EAST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = EAST And UserList(UserIndex).Char.Heading = NORTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = EAST And UserList(UserIndex).Char.Heading = SOUTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = WEST And UserList(UserIndex).Char.Heading = NORTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf Npclist(NpcIndex).Char.Heading = WEST And UserList(UserIndex).Char.Heading = SOUTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    End If
...
Else
    ...
    If UserList(VictimIndex).Char.Heading = NORTH And UserList(UserIndex).Char.Heading = NORTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = SOUTH And UserList(UserIndex).Char.Heading = SOUTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = EAST And UserList(UserIndex).Char.Heading = EAST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = WEST And UserList(UserIndex).Char.Heading = WEST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 2, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 2, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = NORTH And UserList(UserIndex).Char.Heading = WEST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = NORTH And UserList(UserIndex).Char.Heading = EAST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = SOUTH And UserList(UserIndex).Char.Heading = WEST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = SOUTH And UserList(UserIndex).Char.Heading = EAST Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = EAST And UserList(UserIndex).Char.Heading = NORTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = EAST And UserList(UserIndex).Char.Heading = SOUTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = WEST And UserList(UserIndex).Char.Heading = NORTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    ElseIf UserList(VictimIndex).Char.Heading = WEST And UserList(UserIndex).Char.Heading = SOUTH Then
        Call AddtoVar(UserList(UserIndex).Stats.MaxHIT, 1, STAT_ANILLOMAXHIT)
        Call AddtoVar(UserList(UserIndex).Stats.MinHIT, 1, STAT_ANILLOMAXHIT)
    End If
...
UserList(UserIndex).Stats.MaxHIT = MaxH
UserList(UserIndex).Stats.MinHIT = MinH

'[/KEVIN]

End Function

Public Sub UserDañoNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer, ByRef b As Boolean)
Dim Daño As Long



Daño = CalcularDaño(UserIndex, 0, NpcIndex)
...
Dim Obj As ObjData
Dim defAnillo As Integer

'[KEVIN]
If UserList(UserIndex).Invent.AnilloEqpObjIndex > 0 Then
    If ObjData(UserList(UserIndex).Invent.AnilloEqpObjIndex).TipoAnillo = 4 Then
    defAnillo = ObjData(UserList(UserIndex).Invent.AnilloEqpObjIndex).MaxModificador
    End If
ElseIf UserList(UserIndex).Invent.Anillo2EqpObjIndex > 0 Then
    If ObjData(UserList(UserIndex).Invent.Anillo2EqpObjIndex).TipoAnillo = 4 Then
    defAnillo = ObjData(UserList(UserIndex).Invent.Anillo2EqpObjIndex).MaxModificador
    End If
Else
    defAnillo = 0
End If
...
Select Case Lugar
  Case bCabeza
        ...
        If UserList(UserIndex).Invent.CascoEqpObjIndex > 0 Then
           ...
           Daño = Daño - absorbido - defAnillo '<---[KEVIN]
...
'[KEVIN]***************************************
    If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 Then
        If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).DosArmas = 1 Then
            Call UserDañoNpc(UserIndex, NpcIndex, b)
...
Else
    If UserList(UserIndex).Counters.AttackCounter > 0 Then
        Exit Sub
    End If
    UserList(UserIndex).Counters.AttackCounter = IntervaloUserPuedeAtacar
End If
...
'[KEVIN]**************************************
    If UserList(AtacanteIndex).Invent.WeaponEqpObjIndex > 0 Then
        If ObjData(UserList(AtacanteIndex).Invent.WeaponEqpObjIndex).DosArmas = 1 Then
            Call UserDañoUser(AtacanteIndex, VictimaIndex, b)
...
Public Sub UserDañoUser(ByVal AtacanteIndex As Integer, ByVal VictimaIndex As Integer, ByRef b As Boolean)
...
If UserList(VictimaIndex).Invent.AnilloEqpObjIndex > 0 Then
    If ObjData(UserList(VictimaIndex).Invent.AnilloEqpObjIndex).TipoAnillo = 4 Then
    defAnillo = ObjData(UserList(VictimaIndex).Invent.AnilloEqpObjIndex).MaxModificador
    End If
End If

If UserList(VictimaIndex).Invent.Anillo2EqpObjIndex > 0 Then
    If ObjData(UserList(VictimaIndex).Invent.Anillo2EqpObjIndex).TipoAnillo = 4 Then
    defAnillo = ObjData(UserList(VictimaIndex).Invent.Anillo2EqpObjIndex).MaxModificador
    End If
End If
'[/KEVIN]*****************************************************


Daño = CalcularDaño(AtacanteIndex, VictimaIndex)
antdaño = Daño
...
Select Case Lugar
  ...
  Case bCabeza
        ...
        If UserList(VictimaIndex).Invent.CascoEqpObjIndex > 0 Then
           ...
           absorbido = RandomNumber(Obj.MinDef, Obj.MaxDef)
           absorbido = absorbido + defbarco + defAnillo '<------[KEVIN]
           Daño = Daño - absorbido
...
If UserList(AttackerIndex).GuildInfo.GuildName = "" Or UserList(VictimIndex).GuildInfo.GuildName = "" Then
    ...
    If Not Criminal(AttackerIndex) And Not Criminal(VictimIndex) Then
        '[KEVIN]*********************
        If UserList(AttackerIndex).Flags.EnTorneo = 0 Then
...
If Not Criminal(VictimIndex) And UserList(AttackerIndex).Faccion.ArmadaReal > 0 Then
    '[KEVIN]**********
    If UserList(AttackerIndex).Flags.EnTorneo = 0 Then
...
'[KEVIN]******************************
If UserList(AttackerIndex).Stats.Matrimonio <> "" Then
    If UserList(AttackerIndex).Stats.Matrimonio = UserList(VictimIndex).Name Then
        If UserList(VictimIndex).Genero = "Hombre" Then
            Call SendData(ToIndex, AttackerIndex, 0, "||No puedes atacar a tu esposo." & FONTTYPE_INFO)
            PuedeAtacar = False
            Exit Function
        Else
            Call SendData(ToIndex, AttackerIndex, 0, "||No puedes atacar a tu esposa." & FONTTYPE_INFO)
            PuedeAtacar = False
            Exit Function
        End If
    End If
...
If UserList(AttackerIndex).GuildInfo.GuildName <> "" Then
    If AttackerIndex <> VictimIndex Then
        If UserList(AttackerIndex).Flags.EnTorneo = 0 Then
            If UserList(AttackerIndex).GuildInfo.GuildName = UserList(VictimIndex).GuildInfo.GuildName Then
                Call SendData(ToIndex, AttackerIndex, 0, "||No puedes atacar a tus compañeros de clan." & FONTTYPE_INFO)
                PuedeAtacar = False
                Exit Function
            End If
        End If
...
Dim ExpMedia As Long
Dim ExpaDar As Long
Dim TotalNpcVida As Long

ExpMedia = Int(Npclist(NpcIndex).GiveEXP / 2)
...
ExpaDar = Int((ElDaño / TotalNpcVida) * ExpMedia)

Npclist(NpcIndex).Flags.ExpCount = Npclist(NpcIndex).Flags.ExpCount - ExpaDar

If Npclist(NpcIndex).Flags.ExpCount <= 0 Then Exit Sub

Npclist(NpcIndex).Flags.ExpDada = Npclist(NpcIndex).Flags.ExpDada - ExpaDar
