...
If res > 9 Then
   ...
   Call ChangeUserChar(ToMap, 0, UserList(UserIndex).Pos.Map, UserIndex, UserList(UserIndex).Char.Body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.Heading, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.CascoAnim, UserList(UserIndex).flags.AdminInvisible, UserList(UserIndex).flags.Invisible)
...
'    Call LogTarea("Sub DoLingotes")
    If UserList(UserIndex).Invent.Object(UserList(UserIndex).flags.TargetObjInvSlot).Amount < 5 Or Not ObjData(UserList(UserIndex).Invent.Object(UserList(UserIndex).flags.TargetObjInvSlot).ObjIndex).ObjType = OBJTYPE_MINERALES Then
...
If UserList(UserIndex).NroMacotas < MAXMASCOTAS Then
    ...
    If Npclist(NpcIndex).flags.Domable <= CalcularPoderDomador(UserIndex) Then
        Dim Index As Integer, MiNPC As npc
...
If res = 3 Then
    If VictimUserIndex <> 0 Then
        ...
        Call SendData(ToIndex, UserIndex, 0, "||Has apuñalado a " & UserList(VictimUserIndex).Name & " por " & (daño * 1.5) & FONTTYPE_FIGHT)
        Call SendData(ToIndex, VictimUserIndex, 0, "||Te ha apuñalado " & UserList(UserIndex).Name & " por " & (daño * 1.5) & FONTTYPE_FIGHT)
    Else
...
If UserList(UserIndex).Stats.MinSta >= UserList(UserIndex).Stats.MaxSta And UserList(UserIndex).Counters.bPuedeMeditar = False Then
