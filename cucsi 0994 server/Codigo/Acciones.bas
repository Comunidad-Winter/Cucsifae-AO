...
End Sub
'[KEVIN]
Sub AccionParaCofre(ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer, ByVal UserIndex As Integer)
On Error Resume Next

Dim MiObj As Obj
Dim wp As WorldPos

If Not (Distance(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y, X, Y) > 1) Then
    
    If ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).Llave = 0 Then
                     
        Call IniciarDepCofre(UserIndex)
    Else
        Call SendData(ToIndex, UserIndex, 0, "||El cofre está cerrado con llave." & FONTTYPE_INFO)
    End If
Else
    Call SendData(ToIndex, UserIndex, 0, "||Estas demasiado lejos." & FONTTYPE_INFO)
End If
...
End Sub
'[KEVIN]
Sub AccionLLaveParaCofre(ByVal UserIndex As Integer, ByVal Slot As Integer)
On Error Resume Next
...
If UserList(UserIndex).Flags.TargetObj = 0 Then Exit Sub
        ...
        If TargObj.ObjType <> OBJTYPE_CONTENEDORES Then Exit Sub
                  ...
                  If TargObj.Llave > 0 Then
                     If TargObj.Clave = Obj.Clave Then
                        ...
                        = ObjData(MapData(UserList(UserIndex).Flags.TargetObjMap, UserList(UserIndex).Flags.TargetObjX, UserList(UserIndex).Flags.TargetObjY).OBJInfo.ObjIndex).IndexAbierta
                        ...
                        Call MakeObj(ToMap, 0, UserList(UserIndex).Flags.TargetObjMap, MapData(UserList(UserIndex).Flags.TargetObjMap, UserList(UserIndex).Flags.TargetObjX, UserList(UserIndex).Flags.TargetObjY).OBJInfo, _
                        UserList(UserIndex).Flags.TargetObjMap, UserList(UserIndex).Flags.TargetObjX, UserList(UserIndex).Flags.TargetObjY)
                        
                        Call SendData(ToIndex, UserIndex, 0, "||Has abierto el cofre." & FONTTYPE_INFO)
                        Exit Sub
                     Else
                  ...
                  Else
                     If TargObj.Clave = Obj.Clave Then
                        ...
                        Call SendData(ToIndex, UserIndex, 0, "||Has cerrado con llave el cofre." & FONTTYPE_INFO)
