...
'   Call LogTarea("Sub MuereNpc")
   ...
   '[KEVIN]
   ExpFinal = Int(MiNPC.GiveEXP / 2)
   '[/KEVIN]
   
   '[KEVIN]
    MiNPC.Flags.ExpDada = MiNPC.Flags.ExpDada - ExpFinal
    If MiNPC.Flags.ExpDada > 0 Then ExpFinal = ExpFinal + MiNPC.Flags.ExpDada
    MiNPC.Flags.ExpDada = 0
    '[/KEVIN]
   ...
   If Userindex > 0 Then ' Lo mato un usuario?
        ...
        '[KEVIN]
        Call AddtoVar(UserList(Userindex).Stats.Exp, ExpFinal, MAXEXP)
        Call SendData(ToIndex, Userindex, 0, "||Has ganado " & ExpFinal & " puntos de experiencia." & FONTTYPE_FIGHT)
        '[/KEVIN]
        ...
        '[KEVIN](QUESTS)
        Call CheckNpcEnemigo(Userindex, NpcIndex)
        '[/KEVIN]*******************
...
On Error GoTo errh
    ...
    If Npclist(NpcIndex).MaestroUser > 0 Then
            ...
            If LegalPos(Npclist(NpcIndex).Pos.Map, nPos.X, nPos.Y) Then
                ...
                Call SendData(ToNPCArea, 0, Npclist(NpcIndex).Pos.Map, "MP" & Npclist(NpcIndex).Char.CharIndex & "," & nPos.X & "," & nPos.Y)
...
'[KEVIN]
Npclist(NpcIndex).Flags.DeQuest = val(GetVar(npcfile, "NPC" & NpcNumber, "DeQuest"))
'[/KEVIN]
...
Npclist(NpcIndex).Flags.ExpCount = Int(Npclist(NpcIndex).GiveEXP / 2)
'[/KEVIN]
